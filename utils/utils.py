...
def get_git_branch():
    try:
        ...
        print(f"Error occurred while getting the branch: {e}")
        return "None"
...
def save_git_patch(patch_file='uncommitted_changes.patch'):
    try:
        with open(patch_file, 'w') as f:
            subprocess.run(['git', 'diff'], stdout=f, check=True)
        print(f"Uncommitted changes saved to {patch_file}")
    except subprocess.CalledProcessError as e:
        print(f"Error occurred while saving the patch: {e}")