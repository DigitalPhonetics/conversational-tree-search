...
from algorithm.schedule.scheduler import ConstantSchedule
...
def get_latest_checkpoint_name(ckpt_path: str) -> str:
    ...
    for file in os.listdir(ckpt_path):
        if ".pt" in file:
            number = int(file.strip("ckpt_").strip(".pt"))
...
if not os.path.exists(f"{path}/tmp"):
    import subprocess
    subprocess.call(['unzip', '-j', zip_path, '-d', f"{path}/tmp"])
...
def remove_newlines_inside_square_brackets(text):
    # Find all instances of text within brackets (including the brackets)
    pattern = r'\[[^\]]*\]'
    matches = re.findall(pattern, text)
    for match in matches:
        # If the matched text contains a newline, replace it and update the original text
        if '\n' in match:
            sanitized_match = match.replace('\n', '')
            text = text.replace(match, sanitized_match)
    return text
...
def load_cfg(cfg):
    ...
    print("Continue learning from step !")    
    model.continue_learning(current_steps=num_timesteps, 
                ...
                current_resets=current_resets,
                total_timesteps=cfg.experiment.algorithm.dqn.timesteps_per_reset, reset_exploration_times=cfg.experiment.algorithm.dqn.reset_exploration_times, 
