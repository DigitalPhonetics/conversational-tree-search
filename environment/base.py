...
class BaseEnv:
    def __init__(self, 
            ...
            noise: float,
            env_id: int = None,
            auto_skip_logic_nodes: bool = True) -> None:
    ...
    def pre_reset(self):
        ...
        self.asked_node_keys = defaultdict(int)
        self.bst = {}
    ...
    def check_user_patience_reached(self) -> bool:
        return self.asked_node_keys[self.current_node.key] > self.user_patience # +1 because  skip action to node already counts as 1 visit
    ...
    def handle_logic_and_varupdate_nodes(self, reward: float) -> Tuple[float, bool, bool]:
        if not self.auto_skip_logic_nodes:
