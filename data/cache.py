...
class Cache:
    ...
    def _encode(self, state_input_key: State, text_embedding_name: str, caching: bool, cache_key: str, encode_fn: Any, value: Union[str, List[str]], noise: float):
        ...
        normalize_embeddings = self.state_embedding_cfg[state_input_key].normalize_embeddings
        query_instruction = self.state_embedding_cfg[state_input_key].query_instruction
        embeddings = encode_fn(value, normalize_embeddings, query_instruction).detach().cpu()
    ...
    def batch_encode_text(self, state_input_key: State, text: List[str], noise: float):
        ...
        # embedding
        embeddings = text_embedding.batch_encode(text, normalize_embeddings, query_instruction) # batch x embedding_dim
    ...
    def batch_encode_answer_text(self, node: List[DialogNode], action_space_dim: int):
        ...
        text_embedding_name = self.state_embedding_cfg[State.ACTION_TEXT]._target_
        text_embedding: TextEmbeddings = self.text_embeddings[text_embedding_name]
        normalize_embeddings = self.state_embedding_cfg[State.ACTION_TEXT].normalize_embeddings
        query_instruction = self.state_embedding_cfg[State.ACTION_TEXT].query_instruction
        ...
        while batch_end_index < len(node):
            ...
            if len(current_batch) > 0:
                # encode
                enc = text_embedding.batch_encode(current_batch, normalize_embeddings, query_instruction).detach().cpu() # batch x embedding_dim
                enc = enc.detach().cpu()
