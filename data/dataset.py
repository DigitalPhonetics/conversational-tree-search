...
class GraphDataset:
    def __init__(self, graph_path: str, answer_path: str, use_answer_synonyms: bool, augmentation: DataAugmentationLevel, augmentation_path: str = None, resource_dir: str = "resources/",
        ...
        self.num_answer_synonyms = sum([len(self.answer_synonyms[answer_key]) for answer_key in self.answer_synonyms])
        ...
        print("- question limit:", question_limit, " - total loaded: ", sum([len(node.questions) for node in self.node_list]))
        print("- answer limit:", answer_limit, " - total loaded: ", sum([len(self.answer_synonyms[answer_key]) for answer_key in self.answer_synonyms]))
    ...
    def _load_graph(self, resource_dir: str, graph_path: str, augmentation: DataAugmentationLevel, augmentation_path: str, question_limit: int):
        ...
        with open(os.path.join(resource_dir, graph_path), "r") as f:
            ...
            self.nodes_by_type: Dict[NodeType, List[DialogNode]] = defaultdict(lambda: [])
            ...
            for dialognode_json in data['nodes']:
                ...
                for index, answer_json in enumerate(dialognode_json['data']['answers']):
                    ...
                    self.answers_by_key[int(answer.key)] = answer
                # sort answers
                ...
                if self._should_load_original_data(augmentation):
                    ...
                    if question_limit > 0 and len(candidates) < question_limit:
                        random.shuffle(candidates)
                        candidates = candidates[:question_limit+1]
        ...
        if use_answer_synonyms and self._should_load_generated_data(augmentation):
            ...
            with open(answer_augmentation_path, "r") as f:
                ...
                for answer_key in generated_answers:
                    candidates = generated_answers[answer_key]
                    if answer_limit > 0 and len(candidates) < answer_limit:
                        random.shuffle(candidates)
                        candidates = candidates[:answer_limit+1]
...
class StandardGraphDataset(GraphDataset):
    ...
    def _load_answer_synonyms(self, resource_dir: str, answer_path: str, use_answer_synonyms: bool, augmentation: DataAugmentationLevel, augmentation_path: str, answer_limit: int):
        ...
        answer_data = {int(answer.key): [answer.text] for answer in self.answers_by_key.values()} # key is also the only possible value
        if not use_answer_synonyms or augmentation == DataAugmentationLevel.ARTIFICIAL_ONLY:
            if not use_answer_synonyms:
                print("- not using synonyms")
            else:
