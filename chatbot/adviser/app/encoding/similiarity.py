...
class AnswerSimilarityEncoding:
    ...
    def batch_encode(self, current_user_utterance: List[str], dialog_node: List[DialogNode], noise: float = 0.0, **kwargs) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        answer_embeddings = [self.similarity_model.encode([answer.text for answer in node.answers], convert_to_tensor=True, show_progress_bar=False) for node in dialog_node] # List (batch), entry: answers x 512
