...
from config import ActionConfig, ActionType, StateConfig
...
class StateEncoding:
    ...
    def __init__(self, cache: Cache, state_config: StateConfig, action_config: ActionConfig, data: GraphDataset) -> None:
        ...
        self.action_masks = self._calculate_action_masks(data)

        print("Space dimensions:", str(self.space_dims))

    def _calculate_action_masks(self, data: GraphDataset) -> Dict[int, torch.FloatTensor]:
        """ Pre-calculate action mask per node s.t. it is usable without database lookups """
        masks = {}
        for node in data.node_list:
            mask = torch.zeros(self.space_dims.num_actions, dtype=torch.bool, device=self.cache.device)
            if node.node_type == NodeType.QUESTION:
                skip_answer_count = len(node.answers)
                assert skip_answer_count <= data.get_max_node_degree()
                if skip_answer_count > 0:
                    mask[skip_answer_count+1:] = True # no invalid SKIP actions (0: ASk, 1,...N: SKIP)
                elif skip_answer_count == 0:
                    mask[1:] = True # no SKIP actions because node has no answers
            elif node.node_type == NodeType.INFO:
                if node.connected_node:
                    mask[2:] = True # no invalid SKIP actions (only 1 skip possible)
                else:
                    mask[1:] = True # no follow-up node -> no SKIP action
    ...
    def get_action_masks(self, node_keys: List[int]) -> torch.FloatTensor:
        return torch.cat([self.action_masks[key] for key in node_keys], dim=0)
