...
from utils.loader import save_config
from utils.utils import AutoSkipMode, to_class

from config import INSTANCES, ActionConfig, InstanceType, StateConfig, WandbLogLevel, register_configs, EnvironmentConfig, DatasetConfig, ActiveLearningConfig
...
CKPT_BASE_DIR = "/fs/scratch/users/vaethdk/cts_activelearning_weights"
CKPT_DIR = f"{CKPT_BASE_DIR}/{run_id}"

OmegaConf.register_resolver("run_dir", lambda : CKPT_DIR)
...
def load_cfg(cfg):
    ...
    if "validation" in cfg.experiment and not isinstance(cfg.experiment.validation, type(None)): 
        val_data, cache, state_encoding, val_env = setup_data_and_vecenv(device=cfg.experiment.device, dataset_cfg=cfg.experiment.validation.dataset, environment_cfg=cfg.experiment.environment,
                                                                        mode="val", n_envs=cfg.experiment.environment.num_val_envs, log_dir=f"{CKPT_DIR}/best_eval/monitor_logs",
                                                                        cache=cache, encoding=state_encoding,
                                                                        state_config=cfg.experiment.state, action_config=cfg.experiment.actions,
                                                                        torch_compile=cfg.experiment.torch_compile,
                                                                        save_terminal_obs=cfg.experiment.algorithm.dqn.save_terminal_obs,
        ...
        callbacks.append(CustomEvalCallback(eval_env=val_env, mode='eval',
                             best_model_save_path=f"{CKPT_DIR}/best_eval/weights",
                             log_path=f"{CKPT_DIR}/best_eval/logs",
    ...
    if "testing" in cfg.experiment and not isinstance(cfg.experiment.testing, type(None)):
        test_data, cache, state_encoding, test_env = setup_data_and_vecenv(device=cfg.experiment.device, dataset_cfg=cfg.experiment.testing.dataset, environment_cfg=cfg.experiment.environment,
                                                                        mode="test", n_envs=cfg.experiment.environment.num_test_envs, log_dir=f"{CKPT_DIR}/best_test/monitor_logs",
                                                                        cache=cache, encoding=state_encoding,
                                                                        state_config=cfg.experiment.state, action_config=cfg.experiment.actions,
                                                                        torch_compile=cfg.experiment.torch_compile,
                                                                        save_terminal_obs=cfg.experiment.algorithm.dqn.save_terminal_obs,
        ...
        callbacks.append(CustomEvalCallback(eval_env=test_env, mode='test',
                        best_model_save_path=f"{CKPT_DIR}/best_test/weights",
                        log_path=f"{CKPT_DIR}/best_test/logs",
    ...
    if cfg.experiment.logging.wandb_log != WandbLogLevel.NONE:
        ...
        run = wandb.init(
            project="cts_activeleraning",
            config=OmegaConf.to_container(cfg, resolve=True) | {"ckpt_path": CKPT_DIR},
        ...
        #     model_save_path=f"{CKPT_BASE_DIR}/{run_id}",
        #     verbose=2)
        # )

    # save git info and config
    save_config(save_path=f"{CKPT_DIR}", omegaconf=cfg)
