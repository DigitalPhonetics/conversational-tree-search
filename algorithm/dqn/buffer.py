...
class CustomReplayBufferSamples(NamedTuple):
    ...
    prediction_head_mask: Optional[th.Tensor]
...
class CustomReplayBuffer:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.bernoulli_p_train_heads = bernoulli_p_train_heads
        if self._need_prediction_head_mask():
            # we only need to mask in case we have more than one prediction head that can be chosen with probability < 1
            self.prediction_head_mask = th.ones(buffer_size, num_prediction_heads) # buffer x #prediction_heads
    ...
    def _need_prediction_head_mask(self) -> bool:
        return self.num_prediction_heads > 1 and self.bernoulli_p_train_heads < 1.0
    ...
    def save_params(self) -> Dict[str, Any]:
        ...
        if self._need_prediction_head_mask():
            params['prediction_head_mask'] = self.prediction_head_mask
    ...
    def load_params(self, data):
        ...
        self.full = data['full']
        if self._need_prediction_head_mask():
            self.prediction_head_mask = data['prediction_head_mask']
    ...
    def add(
        ...
        if end_pos > self.capacity:
            ...
            overflow = end_pos - self.capacity # number of elements in the batch that don't fit and have to be reassigned to beginning of buffer
            # add fitting part to end of buffer 
            self.add(obs[:-overflow], next_obs[:-overflow], action[:-overflow], reward[:-overflow], done[:-overflow], infos[:-overflow], is_aritificial)
            # add non-fitting part to beginning of buffer
            self.add(obs[-overflow:], next_obs[-overflow:], action[-overflow:], reward[-overflow:], done[-overflow:], infos[-overflow:], is_aritificial)
        else:
            # does fit - add batch
            ...
            # Create prediction head mask for bootstrapped model with multiple prediction heads
            if self._need_prediction_head_mask():
                self.prediction_head_mask[self.pos:end_pos] = th.bernoulli(th.full((batch_size, self.num_prediction_heads), fill_value=self.bernoulli_p_train_heads))
            # save memory
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> CustomReplayBufferSamples:
        ...
        return CustomReplayBufferSamples(
            ...
            self.prediction_head_mask[batch_inds].clone().detach().to(self.device) if self._need_prediction_head_mask() else None
        )
...
class PrioritizedReplayBuffer(CustomReplayBuffer):
    def __init__(self,
            ...
            bernoulli_p_train_heads: float = 1.0,
            **kwargs):
        super().__init__(buffer_size=buffer_size, observation_space=observation_space, action_space=action_space, device=device,
                         num_prediction_heads=num_prediction_heads, bernoulli_p_train_heads=bernoulli_p_train_heads,**kwargs)
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> PrioritizedReplayBufferSamples:
        ...
        for i in range(batch_size):
            ...
            if data_index < self.tree.n_entries:
                ...
                if self._need_prediction_head_mask():
                    prediction_head_masks.append(self.prediction_head_mask[data_index].clone().detach().unsqueeze(0))
            else: 
                # invalid index
                continue
        ...
        while len(indices) < batch_size:
            ...
            if empty:
                ...
                if self._need_prediction_head_mask():
                    prediction_head_masks.append(self.prediction_head_mask[rand_idx].clone().detach().unsqueeze(0))
            else:
        ...
        return PrioritizedReplayBufferSamples(
            ...
            weights,
            indices,
            prediction_head_masks if self._need_prediction_head_mask() else None
        )
...
class PrioritizedLAPReplayBuffer(PrioritizedReplayBuffer):
    def __init__(self,
            ...
            bernoulli_p_train_heads: float = 1.0,
            **kwargs):
        super().__init__(buffer_size=buffer_size, observation_space=observation_space, action_space=action_space, alpha=alpha, beta=beta, device=device,
                         num_prediction_heads=num_prediction_heads, bernoulli_p_train_heads=bernoulli_p_train_heads,**kwargs)
