...
class StandardTarget(DQNTarget):
    ...
    def target(self, next_q_values: torch.FloatTensor, data: CustomReplayBufferSamples, q: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # TODO: I commented this out - do we need to reshape here??
...
class MuenchausenTarget(DQNTarget):
    ...
    def target(self, next_q_values: torch.FloatTensor, data: CustomReplayBufferSamples, q_old: torch.FloatTensor) -> torch.FloatTensor:
        ...
        sum_term = softmax(next_q_values / self.tau, dim=-1) * (next_q_values - self._munchausen_stable_logsoftmax(next_q_values, self.tau)) # batch (x num_heads) x actions
        log_policy = self._munchausen_stable_logsoftmax(q_old, self.tau)
        
        if next_q_values.dim() == 2:
            # standard models
            log_policy = log_policy.gather(-1, data.actions).view(-1) # batch x actions -> batch
            ...
            log_policy = log_policy.gather(-1, data.actions.view(batch_size,1).repeat(1, num_prediction_heads).unsqueeze(-1)) # batch x num_heads x actions -> batch x num_heads x 1
            log_policy = log_policy.squeeze(-1) # batch x num_heads
        ...
        return rewards + self.alpha*log_policy + self.gamma * sum_term.masked_fill(~mask, 0.0).sum(-1) * (1.0 - dones)
