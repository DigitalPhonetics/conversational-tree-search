...
CHECKPOINT_LOCATION = "/mount/arbeitsdaten/asr-2/vaethdk/adviser_reisekosten/newruns/V8_GBERT_25NOISE_50DROPOUT_ACTIONPOS_dqn_50dialog_1_gbert-large_nouser_intent_prediction__12345678__1665687861/ckpt_460000.pt"
WANDB_ID = "1uwzetk8"
...
class Trainer:
    def setUp(self) -> None:
        ...
        seed = 12345678
        self.exp_name_prefix = "CONTINUED_V8_GBERT_25NOISE_50DROPOUT_ACTIONPOS"
   
        self.args = {
            "spaceadapter": {
                "configuration": SpaceAdapterConfiguration(
                    text_embedding="gbert-large", #'distiluse-base-multilingual-cased-v2', # 'gbert-large' # 'cross-en-de-roberta-sentence-transformer',
        ...
        if EXPERIMENT_LOGGING != ExperimentLogging.NONE:
            ...
            os.mkdir(f"/mount/arbeitsdaten/asr-2/vaethdk/adviser_reisekosten/newruns/{self.run_name}")
            os.mkdir(f"/fs/scratch/users/vaethdk/adviser_reisekosten/newruns/{self.run_name}")
        ...
        #
        if self.algorithm == 'dqn':
            self.model = self._dqn_model_from_args(self.args)
            self.target_network = self._dqn_model_from_args(self.args)
    ...
    def log_train_step(self, global_step: int, train_step: int, epsilon: float, timesteps_per_reset: int, beta: float):
        if train_step % 50 == 0 and EXPERIMENT_LOGGING != ExperimentLogging.NONE:
            log_dict = {"train/learning_phase": global_step // timesteps_per_reset}
            if self.algorithm == "dqn":
    ...
    def train_loop(self, global_step: int, train_counter: int, episode_counter: int):
        ...
        while global_step < total_timesteps:
            ...
            for done_idx, done in enumerate(dones):
                ...
                if evaluation and global_step % eval_every_train_timesteps == 0:
                    self.eval(self.eval_env, eval_dialogs, global_step, prefix="eval")
                    eval_goal_asked_score = self.eval(self.test_env, eval_dialogs, global_step, prefix="test")
...
def load_ckpt(trainer: Trainer):
    ...
    trainer.target_network.load_state_dict(ckpt['model'])
    trainer.model.to(trainer.device)
    trainer.target_network.to(trainer.device)
    trainer.optimizer.load_state_dict(ckpt['optimizer'])

    global_step = ckpt['global_step']
    episode_counter = ckpt['episode_counter']
    train_counter = ckpt['train_counter']

    torch.set_rng_state(ckpt['torch_rng'])
    np.random.set_state(ckpt['numpy_rng'])
    random.setstate(ckpt['rand_rng'])

    return global_step, train_counter, episode_counter
...
if __name__ == "__main__":
    ...
    global_step, train_counter, episode_counter = load_ckpt(trainer)
    # trainer.fill_replay_buffer(global_step)
   
    if trainer.algorithm == "dqn":
        trainer.train_loop(global_step, train_counter, episode_counter)
